  ...
  <div class="col-md-10 pl-5 res mb-5 mt-3">
    <div class="mt-4 w-75 width-100" style="margin: 0 auto">
      ...
      <div class="bg-16 p-3 mb-4">
          ...
          <tbody>
            <tr>
              ...
              <td colspan="2">
                <input
                  type="text"
                  placeholder="Nhập tên chương trình combo"
                  class="w-100"
                  v-model="addCombo.title"
                />
              </td>
              <!-- <td style="width: 15%;" class="font-weight-bold text-right">
                Loại voucher
              </td>
              <td>
                <select
        ...
        <h6 class="font-weight-bold color-main">THÔNG TIN COMBO</h6>
        <div class="mt-1">
          <table class="table table-borderless">
            <tbody>
              <tr>
              ...
              <tr>
                ...
                <td>
                  <select
                    ...
                    v-model="addCombo.type_discount"
                  >
                ...
                <td colspan="2" v-if="addCombo.type_discount != ''">
                  <div class="row">
                    <div class="col-5 font-weight-bold text-right pt-2">
                      Nhập mức giảm:
                    </div>
                    <div class="col-7">
                      <div v-if="addCombo.type_discount == 1">
                        <div class="right-inner-addon input-container">
                          ...
                          <input
                            ...
                            placeholder="Nhập %"
                            class="w-100"
                            v-model="addCombo.discount"
                          />
                        </div>
                      </div>
                      <div v-if="addCombo.type_discount == 2">
                        <div class="right-inner-addon input-container">
                          <span>VNĐ</span>
                          <input
                            type="text"
                      ...
                      <div v-if="addCombo.type_discount == 3">
                        <select
                          class="inputx mr-3 res-margin-0 w-100"
                          style="outline: none; style: 100%"
        ...
        <div class="text-center">
          <div class="mt-3 text-center pb-3">
            ...
            <span @click="submitAddCombo" class="borderx color-main">
              Lưu và hiển thị
            </span>
          </div>
        </div>
  ...
  methods: {
    ...
    submitAddCombo() {
      if (this.product_bonus) {
        this.addCombo.product_bonus = this.product_bonus;
      }
      HTTP.post(`/salon/main/manage/marketing/2/store`, this.addCombo).then(
        (res) => {
          let result = res.data;
          if (result.status === 1) {
    ...
    getListProductAndService() {
      HTTP.get(
        `/salon/main/manage/form/${this.addCombo.salon_id}/commission`
      ).then((res) => {
        let result = res.data;
        if (result.status === 1) {
    ...
    onSelect(product) {
      if (this.addCombo.product_id.length == 0) {
        this.addCombo.product_id += product.id;
      } else this.addCombo.product_id += "," + product.id;
    },
    backPage() {
      history.back();
    },
