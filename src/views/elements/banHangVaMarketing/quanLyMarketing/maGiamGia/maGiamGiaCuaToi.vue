<template>
  <div class="col-md-10 res mt-3">
    ...
    <div class="mt-3">
      <div class="bg-16 p-3">
        <h6 class="color-main font-weight-bold">KHUYẾN MÃI TRÊN APP SALON</h6>
          ...
          >
            ...
            <tbody v-if="listDiscount.length > 0">
              <tr v-for="(discount, index) in listDiscount" :key="index">
                <td v-if="discount.type_discount == 1">
                  Giảm tiền
                </td>
                <td v-else>Giảm phần trăm</td>
                <td>
                  {{ discount.title }} <br />
                  Code: <b> {{ discount.code }} </b>
                </td>
                <td v-if="discount.type_voucher == 0">
                  Giảm toàn shop
                </td>
                <td v-else>
                  Giảm theo sản phẩm
                ...
                <td v-else>{{ discount.discount }} %</td>
                ...
                <td>
                  ...
                  <span>
                    ...
                    {{ formatDate(discount.end_date) }}</span
                  >
                </td>
                <td>
                  <span>
                    <router-link
                      :to="{
                        ...
                        params: { idDiscount: discount.id },
                      }"
                    >
                      <span class="color-main">Chi tiết</span>
                    </router-link> </span
                  ><br />
                  <!-- <router-link to="/quanLyMarketing/chuongTrinhCuaToi/donHang"> -->
                  <!-- <span class="color-main">Đơn hàng</span> -->
                  <!-- </router-link> -->
                  <div>
                    <span
                      class="color-main"
  ...
  methods: {
    deleteDiscount(idDiscount) {
      this.$swal({
        ...
        if (result.value) {
          HTTP.delete(
            `/salon/main/manage/marketing/1/${idDiscount}/delete?type_offline=2`
          ).then((res) => {
            let result = res.data;
            if (result.status === 1) {
    ...
    getListDiscount() {
      ...
      HTTP.get(
        "/salon/main/manage/marketing/1?type_offline=2&salon_id=" +
          this.salon_id +
          "&page=" +
          page
      ).then((res) => {
