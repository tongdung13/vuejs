  ...
  <div class="col-md-10 res mb-5 mt-3">
    ...
    <div class="mt-4">
      <div class="mt-3">
        ...
        >
          <span>PHIẾU THU</span></router-link
        >
        <router-link
    ...
    <div class="mt-3 bg-16">
      <div class="p-3">
        <div class="row">
          <div class="col">
            <h5 class="color-main">PHIẾU CHI</h5>
          </div>
          <div class="col text-right">
            <router-link :to="{ name: 'themPhieuChi' }">
              <span class="hover inputx color-main">Thêm phiếu chi</span>
            </router-link>
          </div>
        </div>
            ...
            <div class="bg5 table-responsive">
              ...
              <td colspan="2">456.321.111,00đ</td>
                  ...
                  <tr v-for="(payment, index) in listPayment" :key="index">
                    ...
                    <td>{{ checkSalonId(payment.salon_id) }}</td>
                    <td>{{ payment.content }}</td>
                    <td>{{ formatPrice(payment.money) }}</td>
                    <td>
                      <router-link
                        :to="{
                          name: 'capNhatPhieuChi',
                          params: { phieuChiId: payment.id },
                        }"
                      >
                      ...
                      >
                      <span
                        @click="deleteReciept(payment.id)"
                        class="text-danger"
                ...
                <tbody v-else class="text-center">
                  <th colspan="9" class="color-main">Chưa có phiếu chi.</th>
                </tbody>
              </table>
            </div>
          </div>
        ...
        <div class="bg6 p-2 mt-4" v-if="hasData && moneyControl == ''">
          <div class="row">
            <div class="col-md-6 col-sm-6 col-6">
              <div class="font-weight-bold color-main">TỔNG CHI THÁNG NÀY:</div>
            </div>
            <div class="col-md-6 col-sm-6 col-6 text-right">
              <div class="font-weight-bold color-main">
        ...
        <div class="bg6 p-2 mt-4" v-if="hasData && moneyControl != ''">
          <div class="row">
            <div class="col-md-6 col-sm-6 col-6">
              <div class="font-weight-bold color-main">TỔNG CHI:</div>
            </div>
            <div class="col-md-6 col-sm-6 col-6 text-right">
              <div class="font-weight-bold color-main">
...
export default {
  ...
  created() {
    HTTP.get(`/common/listSalon?page=1`).then((res) => {
      this.listSalon = res.data.data.data;
      // console.log(this.listSalon);
    });
  ...
  methods: {
    ...
    listPayments() {
      ...
      // alert(this.salon_id);
      HTTP.get(
        `/salon/main/manage/payments?salon_id=${this.salon_id}&page=${this.paginate.currentPage}&start_date=${this.startDate}&end_date=${this.endDate}`
      ).then((res) => {
        let result = res.data;
        if (result.status === 1) {
          let data = result.data;
          if (data.data.data.length > 0) {
            this.hasData = true;
            this.listPayment = data.data.data;
            ...
            // console.log(this.listReceipt);
          } else {
            this.paginate.totalPage = 0;
            this.hasData = false;
          }
        } else {
          this.msg = "Chưa có phiếu chi";
        }
        this.loading = false;
      });
    },
    ...
    deleteReciept(paymentId) {
      this.$swal({
        ...
        if (result.value) {
          HTTP.delete(`/salon/main/manage/payments/${paymentId}/delete`).then(
            (res) => {
              let result = res.data;
              if (result.status === 1) {
