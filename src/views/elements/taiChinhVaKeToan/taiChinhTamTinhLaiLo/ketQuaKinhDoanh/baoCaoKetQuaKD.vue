  ...
  <div class="bg-8 pt-3">
    <div class="text-center" style="position: relative">
      <span style="font-size: 15px; font-weight: bold"
        >BẢNG BÁO CÁO KẾT QUẢ KINH DOANH THÁNG {{ month }}</span
      ><br />
      <div>
        <div>
        ...
        >
          <span class="dropdown" style="float: right; margin-top: -6px">
            ...
            <div class="dropdown-content">
              ...
              <div>
                <router-link :to="{ name: 'taoBangKetQuaKinhDoanh' }"
...
export default {
  ...
  methods: {
    ...
    listFinanceAccountings() {
      HTTP.get(`/salon/main/manage/finance-accounting?type=1`).then((res) => {
        ...
        if (result.status === 1) {
          ...
          this.month = this.listFinanceAccounting[1].month;
    ...
    detailRowTable() {
      HTTP.get(
        `/salon/main/manage/finance-accounting/${this.tableId}?type=1&salonId=1`
      ).then((res) => {
        let result = res.data;
        // console.log(result);
        if (result.status === 1) {
    ...
    submitUpdateTable() {
      HTTP.put(
        `/salon/main/manage/finance-accounting/${this.tableId}/update?type=1&salonId=1`,
        this.updateTable
      ).then((res) => {
        let result = res.data;
        if (result.status === 1) {
