  ...
  <div class="bg-8 pt-3">
    <div class="text-center" style="position: relative">
      <span style="font-size: 15px; font-weight: bold"
        >BẢNG BÁO CÁO LƯU CHUYỂN TIỀN TỆ</span
      ><br />
      <div>
        <div>
    ...
    <div class="table-responsive mt-2" style="border-radius: 16px">
      ...
      <!-- sửa thông báo -->
      <div class="modal fade" id="update3" role="dialog">
...
export default {
  ...
  methods: {
    ...
    listFinanceAccountings() {
      HTTP.get(`/salon/main/manage/finance-accounting?type=3`).then((res) => {
        let result = res.data;
        // console.log(result);
        if (result.status === 1) {
    ...
    detailRowTable() {
      HTTP.get(
        `/salon/main/manage/finance-accounting/${this.tableId}?type=3&salonId=1`
      ).then((res) => {
        let result = res.data;
        // console.log(result);
        if (result.status === 1) {
    ...
    submitUpdateTable() {
      HTTP.put(
        `/salon/main/manage/finance-accounting/${this.tableId}/update?type=3&salonId=1`,
        this.updateTable
      ).then((res) => {
        let result = res.data;
        if (result.status === 1) {
